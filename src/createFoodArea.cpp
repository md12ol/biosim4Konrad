...
// This generates food area points, which are grid locations with value
// FOODAREA. A list of food area locations is saved in private member
// Grid::foodAreaLocations and, for some scenarios, Grid::foodAreaCenters.
// Those members are available read-only with Grid::getFoodAreaLocations().
...
void Grid::createFoodArea(unsigned foodAreaType)
{
    foodAreaLocations.clear();
    foodAreaCenters.clear();  // used only for some food area types
...
//                              (int16_t)randomUint((int)radius + margin, p.sizeY - ((float)radius + margin)) );
            ...
            foodAreaCenters.push_back(center0);
            //foodAreaCenters.push_back(center1);
